...
class ActionClassifierConfig:
    ...
    window_size: int = 30  # 30 frames at 30fps = 1 second
...
class ProcessSegmenterConfig:
    ...
    idle_timeout_frames: int = 90  # 3 seconds at 30fps
...
def load_app_config(config_path: Optional[str] = None) -> AppConfig:
    ...
    config.ai = AiConfig(
        api_key=ai_cfg.get("api_key", os.environ.get("DEEPSEEK_API_KEY", config.ai.api_key)),
    ...
    config.auth = AuthConfig(
        jwt_secret_key=auth_cfg.get(
            "jwt_secret_key",
            os.environ.get("JWT_SECRET_KEY", config.auth.jwt_secret_key),
        ),
    ...
    config.influxdb = InfluxDBConfig(
        ...
        token=influx_cfg.get("token", os.environ.get("INFLUXDB_TOKEN", config.influxdb.token)),
