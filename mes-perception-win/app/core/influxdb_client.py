...
class InfluxDBClient:
    ...
    def _write_with_retry(self, bucket: str, record: Any) -> bool:
        ...
        try:
            ...
            with self._retry_lock:
                self._retry_count += 1
                current_retries = self._retry_count
            if current_retries <= self._max_retries:
                # Exponential backoff: 1s, 2s, 4s
                wait_time = 2 ** (current_retries - 1)
                logger.warning(
                    "InfluxDB write failed (attempt %d/%d), retrying in %ds: %s",
                    current_retries, self._max_retries, wait_time, e,
                )
                ...
                try:
                    ...
                    logger.error("InfluxDB write retry failed: %s", retry_err)
            else:
                logger.error(
