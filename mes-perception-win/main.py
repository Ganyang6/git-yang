...
def _publish_video_progress(
    ...
    for attempt in range(max_retries):
        try:
            _safe_redis_call(
                lambda: (adapter._client.client.publish(
            ...
            _safe_redis_call(
                lambda: adapter._client.client.xadd(
                    "mes:video_progress",
                    {
                        "task_id": task_id,
    ...
    try:
        ...
        while True:
            ...
            while True:
                ...
                # Use grab()+retrieve() instead of cap.read() to avoid
                # OpenCV H.264 decoder blocking on certain keyframes.
                if not cap.grab():
                    break
